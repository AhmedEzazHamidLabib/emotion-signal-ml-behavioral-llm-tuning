...
CONFIG = {
    "dataset_train_path": "data/go_emotions_annotated_train",
    "dataset_val_path": "data/go_emotions_annotated_validation",
    "dataset_test_path": "data/go_emotions_annotated_test",
    "prototype_path": "emotion_prototypes.json",
...
print("\nBaseline model saved to models/baseline_tfidf_ridge.pkl")
...
for i, sig in enumerate(SIGNALS):
    ...
    per_signal_validation.append({
        "signal": sig,
        "mae": float(sig_mae),
        "rmse": float(sig_rmse),
        "r2": float(sig_r2),
        "accuracy": float(sig_acc),
        "precision_macro": float(sig_precision),
        "recall_macro": float(sig_recall),
...
results = {
    "validation": {
        ...
        "precision_weighted": float(val_precision_weighted),
        "recall_weighted": float(val_recall_weighted),
...
results["per_signal_validation"] = per_signal_validation

with open("models/baseline_tfidf_ridge_results.json", "w") as f:
    json.dump(results, f, indent=2)

print("Saved results to models/baseline_tfidf_ridge_results.json")